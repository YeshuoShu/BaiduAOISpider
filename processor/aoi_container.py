...
class AOI(object):
    ...
    def _not_too_big_or_too_small(self) -> bool:
        return (self.area >= Repo._min_aoi_area)\
               and (self.area <= Repo._max_aoi_area)
    ...
    def _not_too_different(self) -> bool:
        ...
        """
        return (Repo._sortings.get('sort_by_similarity') == 0)\
               or (self.similarity >= Repo._min_similarity)
...
class AOIContainer(object):
    ...
    def mold(cls) -> None:
        cls._df = pd.DataFrame({
            'aoi_list': [AOI_list(idx) for idx in Repo.file.index]
        })
